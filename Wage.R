...
# Combine all names except emp, year, id)
all_variables_names <- setdiff(names(data_2010), c("id", "year"))
...
# Hourly wage
df <- df %>%
  mutate(hwage = (wage / hours))

# Log of Hourly wage
df <- df %>%
  mutate(hwage = log(hwage))

# Drop 'wage' and 'hours' columns
df <- subset(df, select = -c(wage, hours))

# Filter rows where hwage is not equal to -Inf or Inf
df <- subset(df, !(hwage == -Inf | hwage == Inf))
